...
from .models import OneTimePassowrd
...
class RegisterUserView(GenericAPIView):
    ...
    def post(self,request):
        ...
        if serializer.is_valid(raise_exception=True):
            ...
            print("want to know user*************",user)
            return Response({
                'data':user,
                'message': f'Thanks for signing up with a password'
...
class VerifyUserEmail(GenericAPIView):
    def post(self, request):
        ...
        try:
            user_code_obj = OneTimePassowrd.objects.get(code=otpcode)
        ...
        except OneTimePassowrd.DoesNotExist:
...
class PasswordResetRequestView(GenericAPIView):
    ...
    def post(self,request):
        ...
        serializer.is_valid(raise_exception=True)
        return Response({'message':"a link hass been sent to your email to reset your password"},status=status.HTTP_200_OK)
...
class LogutUserView(GenericAPIView):
